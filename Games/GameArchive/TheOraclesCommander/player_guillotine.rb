...
word_prompt = ai_choice.tr alph_1, alph_2
...
loop do
  if guesses > max
    ...
    image_status = File.read("_data/images/guillotined/death.txt")

    puts image_status

    puts "Sister Order ran out of guesses."

    system("echo 'Executed by guillotine...'")
    ...
    if guesses == 1
      image_status = File.read("_data/images/guillotined/mild.txt").strip

      puts image_status
    elsif guesses == 2
      image_status = File.read("_data/images//guillotined/minor.txt").strip

      puts image_status
    elsif guesses == 3
      image_status = File.read("_data/images/guillotined/moderate.txt").strip

      puts image_status
    elsif guesses == 4
      image_status = File.read("_data/images/guillotined/critical.txt").strip

      puts image_status
    elsif guesses == 5
      image_status = File.read("_data/images/guillotined/severe.txt").strip
